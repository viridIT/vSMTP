...
pub mod test_types {
    ...
    fn test_time() {
        ...
        let re =
            RuleEngine::new("./src/rules/tests/types/time").expect("couldn't build rule engine");
        let mut state = get_default_state();

        state.add_data("custom_time", std::time::SystemTime::UNIX_EPOCH);
    ...
    fn test_socket() {
        ...
        let re =
            RuleEngine::new("./src/rules/tests/types/socket").expect("couldn't build rule engine");
        let mut state = get_default_state();
